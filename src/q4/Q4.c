...
{
	// add your code here
	if(ll->size == 0) return; // Empty List
	int mid = ll->size/2;
	...
	ListNode* frontListNodeTail = ll->head;
	for (int i = 0; i < mid - 1; i++) {
		frontListNodeTail = frontListNodeTail->next;
	}

	ListNode* backListHeadNode = frontListNodeTail->next;
	resultFrontList->head = ll->head;
	resultFrontList->size = mid;
	frontListNodeTail->next = NULL;

	resultBackList->head = backListHeadNode;
